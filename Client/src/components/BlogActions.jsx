...
const BlogActions = ({ upvotes, downvotes, onReport, blogId, blogTitle, blog, onVoteUpdate }) => {
  ...
  // Fetch user's vote status and latest vote counts on mount
  useEffect(() => {
    const fetchUserVote = async () => {
      if (!user) {
        setVotes({ up: upvotes || 0, down: downvotes || 0, userVote: null });
        return;
      }
      try {
        ...
        setVotes({
          up: blogData.upvotes.length,
          down: blogData.downvotes.length,
    ...
    fetchUserVote();
  }, [user, blogId, api, upvotes, downvotes]);
  ...
  const handleVote = async (type) => {
    ...
    try {
      ...
      setVotes({
        ...
        userVote: type === 'up' ? (response.data.upvotes > votes.up ? 'up' : null)
          : (response.data.downvotes > votes.down ? 'down' : null)
      });

      if (onVoteUpdate) onVoteUpdate(); // Trigger parent refresh
